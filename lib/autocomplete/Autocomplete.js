...
var factory = function factory(Chip, Input) {
  var Autocomplete = function (_Component) {
    ...
    _createClass(Autocomplete, [{
      ...
      key: 'select',
      value: function select(event, target) {
        _events2.default.pauseEvent(event);
        var values = this.values(this.props.value);
        var newValue = target === void 0 ? event.target.id : target;
        this.handleChange([newValue].concat(_toConsumableArray(values.keys())), event);
      }
    }, {
      key: 'unselect',
      ...
      value: function render() {
        ...
        var keepFocusOnChange = _props.keepFocusOnChange;
        var showSuggestionsWhenValueIsSet = _props.showSuggestionsWhenValueIsSet;
        var theme = _props.theme;

        var other = _objectWithoutProperties(_props, ['allowCreate', 'error', 'label', 'source', 'suggestionMatch', 'selectedPosition', 'keepFocusOnChange', 'showSuggestionsWhenValueIsSet', 'theme']);
